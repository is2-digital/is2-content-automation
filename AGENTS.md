...
The sandbox cannot read `.env`, so all `make` targets (which use `docker compose`) fail with "permission denied". Use `docker exec` instead:

```bash
# Preflight: verify containers are running
docker ps --format "table {{.Names}}\t{{.Status}}" | grep ica
# Must see ica-app-1, ica-postgres-1, ica-redis-1 all "Up"
# If not running, ask the user to run `make dev` in a separate terminal.

# Tests
docker exec ica-app-1 python -m pytest tests/                    # All tests
docker exec ica-app-1 python -m pytest tests/test_pipeline/      # One directory
docker exec ica-app-1 python -m pytest tests/ -k test_name       # Match by name

# Linting & type checking
docker exec ica-app-1 ruff check .                               # Ruff linter
docker exec ica-app-1 ruff format .                              # Ruff auto-format
docker exec ica-app-1 mypy ica/                                  # mypy (strict)

# App commands
docker exec ica-app-1 python -m ica run                          # Trigger pipeline
docker exec ica-app-1 python -m ica status                       # Pipeline status
...
docker exec ica-app-1 alembic -c alembic.ini revision --autogenerate -m "description"
docker exec ica-postgres-1 psql -U ica -d n8n_custom_data        # psql shell
```

### Human use (outside sandbox)

Run `make help` for all available targets (`make test`, `make lint`, `make dev`, etc.).
...
- `ica/scheduler.py` — APScheduler jobs for article collection and pipeline triggers.
...
- **Ruff**: line-length 99, target Python 3.12, rules: E, F, I, N, UP, B, SIM, RUF. RUF001/002/003 are intentionally ignored — unicode chars (en-dash etc.) are used in prompts and Slack messages.
